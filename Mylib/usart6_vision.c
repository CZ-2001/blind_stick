...
void USART6_IRQHandler(void)
{
	if (USART_GetITStatus(USART6, USART_IT_RXNE) != RESET)
	{
		...
		if (ishead6_f == 1)
		{
			...
			if ((sta6 == 2) && (start == 0))
			{
				if ((USART6_RX_BUF[LEN6 - 2] == 0xDF)&&(USART6_RX_BUF[1] == 0x11))
				{
					
					memset(&USART6_RX_BUF, 0, 4);start=1;sta6=0;ishead6_f=0;
				}
			}
			else if ((USART6_RX_BUF[LEN6 - 2] == 0xDF) && (start == 1))
			{
				memset(&USART6_RX_BUF, 0, 4);start=1;sta6=0;ishead6_f=0;
			}
			else if ((sta6 == LEN6 - 1) && (start == 1))
			{
				ishead6_f = 0;
				if (USART6_RX_BUF[LEN6 - 1] == 0xDF)
				{
					USART3_SendBuff(USART6_RX_BUF, 4);
				}

				else memset(&USART6_RX_BUF, 0, 4);
		...
		else
		{
			sta6 = 0;
			USART6_RX_BUF[sta6] = USART_ReceiveData(USART6);
			ishead6_f = (USART6_RX_BUF[sta6] == 0xAB ? 1 : 0);
			if(ishead6_f==0)
			{					memset(&USART6_RX_BUF, 0, 4);
